            ...
            >
              {posts.map((post, i) => (
                <Link
                  key={post.id}
                  href={`/blog/${post.slug}`}
                  className="group shrink-0 w-full sm:w-[calc(50%-12px)] lg:w-[calc(33.333%-16px)]"
                >
                  <motion.div
                    initial={{ opacity: 0, y: 20 }}
                    whileInView={{ opacity: 1, y: 0 }}
                  ...
                  >
                    ...
                    <div className="p-6">
                      ...
                      <h3 className="font-bold text-gray-900 group-hover:text-primary transition-colors duration-300 leading-tight">
                        {post.title}
                      </h3>
              ...
              {posts.map((_, i) => (
                <button
                  ...
                  className={`h-1.5 rounded-full transition-all duration-500 ${
                    active === i ? "bg-primary w-8" : "bg-gray-300 w-2"
                  }`}
                />
              ))}
            </div>
