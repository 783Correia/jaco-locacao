...
const MODELS: Model[] = [
    ...
    { name: "Telescópica 28m", fullName: "Plataforma Telescópica Diesel 28m 4x4", description: "Maior alcance da frota com 28 metros. Para trabalhos em grande altura: torres, fachadas e estruturas elevadas.", category: "articulada", image: "/frota/plataforma-telescopica-diesel-28m.png", video: "/frota/plataforma-telescopica-video.mp4", cardSpecs: { "Altura Trab.": "28m", "Alc. Horiz.": "22,9m", "Capacidade": "227kg", "Peso": "16732kg", "Dimensões": "2,49x12,19x3,05m" } },
];
        ...
        <main className="min-h-screen bg-[#0a0a0a] text-white font-sans antialiased overflow-x-hidden">
            ...
            <section className="relative w-full overflow-hidden bg-[#0a0a0a]">
                ...
                <div className="relative z-10 pt-20 pb-12 text-center max-w-4xl mx-auto px-6">
                    ...
                    <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.5 }}
                        className="text-lg md:text-xl max-w-xl mx-auto mb-10 leading-relaxed text-gray-500">
                        ...
                        {HERO_MACHINES.map((m) => (
                            <a key={m.name} href={`${WA}Olá!%20Tenho%20interesse%20na%20${encodeURIComponent(m.name)}.`} target="_blank" rel="noopener noreferrer"
                                ...
                                <div className="px-5 py-4 bg-white/[0.06]">
                                    ...
                                    <p className="text-xs text-gray-500 mb-3">{m.type}</p>
                                    <div className="text-lg font-extrabold text-primary">{m.height}</div>
                                    <div className="text-[10px] uppercase tracking-wider text-gray-500">Altura</div>
                                </div>
                            </a>
                        ))}
                    </div>
                        ...
                        <AnimatePresence mode="popLayout">
                            {filtered.map((m) => (
                                <motion.div key={m.name} layout initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.97 }} transition={{ duration: 0.35 }}
                                    ...
                                    {/* Image / Video */}
                                    <div className="relative aspect-[4/3] w-full bg-white flex items-center justify-center p-4 border-b border-gray-100 overflow-hidden">
                                        {m.video ? (
                                            <video
                                                src={m.video}
                                                autoPlay
            ...
            <footer className="py-6 bg-[#0a0a0a]">
                <div className="max-w-6xl mx-auto px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <Image src="/logos/logo-jaco.png" alt="Jaco Locação" width={120} height={36} className="h-7 w-auto opacity-70" />
                    <div className="flex items-center gap-5">
                        ...
                        <a href="tel:+554830939355" className="text-white/40 hover:text-white transition-colors">
